...
extern float maximum_temperature;
...
static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req)
{
    ...
    float temperatura = ntc_get_temperature();

    sprintf(dhtSensorJSON,
            "{\"temp\":\"%.1f\"}",
            temperatura);
...
static esp_err_t http_server_fan_mode_handler(httpd_req_t *req)
{
    ...
    {
        fan_set_mode(mode->valueint);
        ESP_LOGI(TAG, "Fan mode = %d", mode->valueint);
    }
...
static esp_err_t http_server_temp_config_handler(httpd_req_t *req)
{
    char buf[200];
    ...
    cJSON *maximum = cJSON_GetObjectItem(root, "maxTemp");

	cJSON *fanMode = cJSON_GetObjectItem(root, "fanMode");
	cJSON *fanManual = cJSON_GetObjectItem(root, "fanManual");

    if(cJSON_IsNumber(desired))
    {
        desired_temperature = desired->valuedouble;
    }

    if(cJSON_IsNumber(maximum))
    {
        maximum_temperature = maximum->valuedouble;
    }

    if(cJSON_IsNumber(fanMode))
	{
		fan_set_mode((uint8_t)fanMode->valueint);
	}
    
	if(cJSON_IsNumber(fanManual))
	{
		fan_set_manual((uint8_t)fanManual->valueint);
	}

	ESP_LOGI(TAG,
			"Nueva configuracion: deseada=%.1f maxima=%.1f modo=%d pwm=%d",
			desired_temperature,
			maximum_temperature,
			fan_get_mode(),
			cJSON_IsNumber(fanManual) ? fanManual->valueint : 0);
...
static esp_err_t http_server_rgb_config_handler(httpd_req_t *req)
{
    char buf[200];
    ...
    cJSON *red =
        cJSON_GetObjectItem(root, "red");

    cJSON *green =
        cJSON_GetObjectItem(root, "green");

    cJSON *blue =
        cJSON_GetObjectItem(root, "blue");

    cJSON *brightness =
        cJSON_GetObjectItem(root, "brightness");
    ...
    {
        rgb_led_set_color(
            (uint8_t)red->valueint,
            (uint8_t)green->valueint,
            (uint8_t)blue->valueint);
    }

    if(cJSON_IsNumber(brightness))
    {
        rgb_led_set_brightness(
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		httpd_uri_t temp_config_json = {
				.uri = "/tempConfig.json",
				.method = HTTP_POST,
				.handler = http_server_temp_config_handler,
				.user_ctx = NULL
		};
        
		httpd_uri_t rgb_config_json = {
				.uri = "/rgbConfig.json",
				.method = HTTP_POST,
				.handler = http_server_rgb_config_handler,
				.user_ctx = NULL
		};
